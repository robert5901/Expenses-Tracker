package com.example.expensestracker.pieChart.views
...
class PieChartView @JvmOverloads constructor(
    ...
    private var pieData: List<PieChartDataUi> = emptyList()
    ...
    fun setPieData(data: List<PieChartDataUi>) {
        pieData = data
        invalidate()
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        var startAngle = 0f
        val total = pieData.sumOf { it.amount }.toFloat()

        // Отрисовка категорий
        pieData.forEachIndexed { index, pieEntry ->
            val sweepAngle = (pieEntry.amount.toFloat() / total) * 360
    ...
    // Проверка нажали ли на диаграмму, а не на пустое место
    private fun isPointInsideCategoryLine(
        touchX: Float,
        touchY: Float,
        centerX: Float,
        centerY: Float,
